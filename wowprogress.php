<?php
/**
 * Plugin Name: WoW Progress fr
 * Description: Un widget qui affiche l'avancee PVE de votre guilde.
 * Author: freevision.sk
 * Version: 1.3.2
 * Author URI: http://www.freevision.sk
 * Text Domain: wowprogress
 * traduction francaise : kam1kaz3 http://mstuning.free.fr
 */
/**
 ...
 */
define( 'WOWPROGRESS_VERSION', '1.2.1' );
...
class wowprogress_widget extends WP_Widget {
	...
	function widget($args, $instance){
		...
		foreach ($this->WoWraids as $raid) {
			...
			if ($exp != $raid['exp']){
				...
				// Start raids list
				echo TAB.'<ul class="expansion" style="display: none">'.NL;
			}
	...
	function print_form_fields($instance){
		...
		echo '<tbody>';
		echo $this->form_text_input("title", __("Titre", "wowprogress"), esc_attr($instance['title']));
		echo $this->form_text_input("guild", __("Guilde", "wowprogress"), esc_attr($instance['guild']), __("nom de votre guilde", "wowprogress"));
		...
		foreach ($this->WoWraids as $raid) {
			...
			echo '<tbody>';
			echo $this->form_checkbox_input($raid['tag']."_show", __("Voir", "wowprogress"), $instance[$raid['tag']."_show"]);
			echo $this->form_checkbox_input($raid['tag']."_expand", __("Developper", "wowprogress"), $instance[$raid['tag']."_expand"]);
			echo '</tbody>';
			...
			echo $this->form_text_input($raid['tag']."_time", __("temps", "wowprogress"), $instance[$raid['tag']."_time"], __("temps du mode defi", "wowprogress"));
			echo '<tr><td colspan="3"><hr /></td></tr>';
